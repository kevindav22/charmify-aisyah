...
const OrderAction = ({ charms, totalPrice }: Props) => {
  const handleOrder = () => {
    ...
    const message = `Halo Kak Saya ingin memesan custom bracelet hasil desain saya.
    Ringkasan desain:
    ${summary}
    Estimasi Total:
    ...
    <div className="overflow-hidden rounded-3xl border border-rose-100 bg-white shadow-sm">
      <div className="flex flex-col gap-4 p-4 md:p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-1">
          <h3 className="text-lg font-bold text-slate-900 md:text-xl">Konfirmasi Hasil Desainmu</h3>
          <p className="max-w-2xl text-sm text-slate-500 md:text-base">Jangan lupa download desain terlebih dahulu, kemudian kirimkan gambar beserta rincian pesanan otomatis melalui WhatsApp.</p>
        </div>
